...
fn deposit_backing_gate_reconciles_and_fails_closed() {
    ...
    if !(tool_present("anvil") && tool_present("forge") && tool_present("cast")) {
        eprintln!(
            "[skip] foundry (anvil/forge/cast) not found — this test needs a real on-chain deposit"
        );
        return;
    }
    ...
    let _guard = AnvilNode::spawn(
        "deposit_backing_gate_reconciles_and_fails_closed",
        PORT,
        &[],
    );
    ...
    let mut bwg = BalanceWitnessGenerator::new(
        channel_id,
        salt,
    ...
    let balance_proof = balance_processor
        .prove_receive_deposit(&dep_witness)
        .unwrap();
    bwg.commit_receive_deposit(&balance_proof, &dep_witness)
        .unwrap();
    ...
    let attestation = ChannelBalanceAttestation {
        balance_proof: proof_bytes.clone(),
    };
    ...
    let regev_pk_digests: Vec<Bytes32> = mkeys
        .iter()
    ...
    bad_bytes[last] ^= 0xFF;
    let bad_att = ChannelBalanceAttestation {
        balance_proof: bad_bytes,
    };
    assert!(
    ...
    // ---- 7. ATOMIC check-and-sign (detail2 §3.1): sign_state_if_backed signs ONLY when the
    // state's settled_tx_chain matches the held intmax balance backing, and never otherwise.
    // ----------------
    assert!(
