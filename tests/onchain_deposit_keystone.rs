...
fn self_driving_real_anvil_deposit_matches_rust() {
    ...
    // Spawns anvil and waits until the node answering on PORT is proven to be the one we spawned
    // (fresh chain, our own process). See tests/anvil_harness/mod.rs for why a plain
    // `cast block-number` poll is not enough. Killed on drop.
    let _guard = AnvilNode::spawn("self_driving_real_anvil_deposit_matches_rust", PORT, &[]);
