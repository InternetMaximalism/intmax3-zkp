...
//!   3. the post-debit `H1' = a_send.balance_state.h1()` is bound into the small block's IMSB, and
//!      a REAL validity proof verifies the bp's `channelStateSig` over `hash(H1', tx_tree_root)`
//!      (§C-7 / §F-2) — the SAME H1' the E-2 produced (the stitch B-1↔B-2);
//!   4. the receiver verifies the tx is INCLUDED in the validity-proven small block
//!      (flowReceive3-1).
...
fn cap(c: &mut Command, l: &str) -> String {
    let o = c.output().unwrap_or_else(|e| panic!("{l}: {e}"));
    assert!(
        o.status.success(),
        "{l}: {}",
...
fn cast(rpc: &str, a: &[&str], l: &str) -> String {
    cap(Command::new("cast").args(a).arg("--rpc-url").arg(rpc), l)
}
...
fn pks_array(keys: &[MemberKeys]) -> [RegevPk; intmax3_zkp::constants::MAX_CHANNEL_MEMBERS] {
    let mut a: [RegevPk; intmax3_zkp::constants::MAX_CHANNEL_MEMBERS] =
        std::array::from_fn(|_| RegevPk::padding());
    for (i, k) in keys.iter().enumerate() {
        a[i] = k.regev_pk.clone();
    }
    a
}
fn structural_sigs(r: &ChannelRecord) -> Vec<MemberSignature> {
    (0..r.member_count)
        .map(|i| MemberSignature {
            member_slot: i,
            pk_g: r.member_pk_gs[i as usize],
            signature: vec![1 + i],
        })
        .collect()
}
...
fn unified_inter_channel_transfer_e2e() {
    if !(tool("anvil") && tool("forge") && tool("cast")) {
        eprintln!(
            "[skip] foundry not found — unified inter-channel E2E needs real on-chain deposits"
        );
        return;
    }
    ...
    // ---- block 1: registration; block 2: REAL on-chain deposit funding channel A ----
    {
        ...
        g.add_channel_registration_keys(A_ID, ck.clone());
        g.add_registration_block(0).expect("reg block");
    }
    ...
    );
    {
        ...
        .unwrap();
        g.add_block(0, &[], 0, Bytes32::default()).unwrap();
    }
    let mut a_bwg =
    ...
    let a_cts = [
        a_ct0.clone(),
        encrypt_amount(&mut crng, &a_pks[1], a_bal[1]).unwrap().0,
        encrypt_amount(&mut crng, &a_pks[2], a_bal[2]).unwrap().0,
    ];
    // Decryption Stage 1: per-active-slot Regev pk digests, in the SAME slot order as `a_cts`
    // (mirrors channel_member.rs:601-605).
    let a_regev_pk_digests: Vec<Bytes32> = a_keys
        .iter()
        .map(|k| Bytes32::from(k.regev_pk.poseidon_digest()))
        .collect();
    let mut a_genesis = assemble_genesis_state_backed(
        &a_record,
        &a_cts,
        &a_regev_pk_digests,
    ...
    let icw = InterChannelSendUpdateWitness {
        ...
        destination_recipient_pk: dest_pk.clone(),
    ...
    let pis = icw
        .verify(&StructuralTransport, &VERIFIER)
        .expect("REAL E-2 inter-channel send verifies");
    ...
    };
    {
        ...
        g.add_block_with_tx_v2(A_ID, &[1], 1, tx_tree_root, Some(tx_v2_witness))
            .expect("small block");
    }
    ...
    let mut last = None;
    {
        ...
        for idx in 1..=g.block_number.as_u64() {
            ...
            let init = if prev.is_none() {
                Some(initial_ext.clone())
            } else {
                None
            };
            let p = bhc
                .prove_block(init, prev.clone(), &w)
    ...
    // ----
    tx_v2_proof
        .verify(&tx_v2, a_id.as_u64(), tx_v2_root_h)
        .expect("receiver TxV2 inclusion (flowReceive3-1)");

    eprintln!(
        "[UNIFIED] OK: real deposit (§F-1) → real E-2 → channelStateSig validity-proven over the SAME H1'={} → receiver inclusion. tx_tree_root={}.",
        h1_prime.to_hex(),
        tx_tree_root.to_hex()
    );
}
