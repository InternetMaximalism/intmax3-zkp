...
// VENDORED CHANGE (intmax3-zkp, P3-3): visibility `pub(crate)` -> `pub` ONLY.
// This adds NO constraints and changes NO logic — it exposes the existing
// whole-permutation constraint evaluator so a wrapper AIR with a wider row
// `[Poseidon2Cols | binding_cols]` can call it on the Poseidon2Cols prefix and
// then add its own cross-row binding constraints. The audited round-constraint
// body below is byte-for-byte upstream. See tasks/poseidon-signature-todo.md P3-3.
pub fn eval<
    AB: AirBuilder,
    LinearLayers: GenericPoseidon2LinearLayers<WIDTH>,
    const WIDTH: usize,
