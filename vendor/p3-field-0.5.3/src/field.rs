...
>(
    ...
    let mut val_chunks = values.as_slice().chunks_exact(CHUNK);
    let mut coeff_chunks = coeffs.as_slice().chunks_exact(CHUNK);
    debug_assert_eq!(val_chunks.len(), coeff_chunks.len());

    // One add per group; runs in parallel with the next group's multiplies.
    let mut acc = A::ZERO;
    for (vc, cc) in val_chunks.by_ref().zip(coeff_chunks.by_ref()) {
        let vc: &[A; CHUNK] = vc.try_into().expect("chunks_exact must yield CHUNK items");
        let cc: &[F; CHUNK] = cc.try_into().expect("chunks_exact must yield CHUNK items");
    ...
    let val_rem = val_chunks.remainder();
    let coeff_rem = coeff_chunks.remainder();
...
pub fn chunked_linear_combination<const CHUNK: usize, A: Algebra<F> + Dup, F: Dup>(
    ...
    let mut val_chunks = values.chunks_exact(CHUNK);
    let mut coeff_chunks = coeffs.chunks_exact(CHUNK);
