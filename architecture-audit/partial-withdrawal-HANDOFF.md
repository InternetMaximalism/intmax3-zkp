...
- `single_withdrawal` derives `Withdrawal{recipient=extract_address_from_recipient(transfer.recipient), amount, nullifier=SettledTransfer::nullifier(channel_id, transfer_index, nonce)}` (single_withdrawal_circuit.rs:369-393). C14-verified canonical.
