...
- 帰結 2: `H2`(送金種別タグ)には abstract2 の「ブロック全体の tx_tree_root」ではなく
  **自 channel small block の `tx_tree_root`** が入る。アトミック性(認可と減算の単一署名)の
  議論は変わらない(§D-3)。
- 帰結 3: ITS(intmax-tx-sender)役は、現実装では **`bp_key_id` を持つ member**
  (`ChannelRecord.bp_key_id ∈ member_key_ids`)が担う。
...
- `CHANNEL_MEMBERS = 3`(固定、§G)。`ChannelRecord::validate()` に
  `member_key_ids.len() == CHANNEL_MEMBERS` を追加する(木の高さ `MEMBER_KEY_TREE_HEIGHT = 8` は
  互換のため不変。leaf は 3 つだけ使う)。
...
| `member_signatures: Vec<MemberSignature>` | [維](ただし署名対象が変わる、下記) |
...
- `signing_digest`(domain `PAY_DOMAIN = 0x494d5041` 維持): preimage を
  `[domain, channel_id, prev_state_digest, enc_amount.digest(), nonce, sender_user_id, recipient_user_id]` に変更。
...
|---|---|---|
| `senderAddr / recipientAddr` | `source_user_id: UserId` / `receiver_deltas[i].receiver_user_id` | [維] |
...
pub fn tx_leaf_hash(tx: &InterChannelTx) -> Bytes32 {
    // hash( hash(TX_LEAF_DOMAIN, source_user_id, sender_delta_ct.digest()),
    //       hash(TX_LEAF_DOMAIN, receiver_user_id, receiver_delta_ct.digest()) )
    // → 送信側・受信側の鍵と lattice 残高変化を両翼で束縛
}
```
...
| 他フィールド | [維](`channel_id, bp_key_id, small_block_number, prev_small_block_root, medium_epoch_hint, close_freeze_nonce`) |

`signing_digest()`(domain `0x494d5342` "IMSB")は変更不要 — preimage に `tx_tree_root`(= H2)と
`state_commitment_root`(= H1′)が**既に両方含まれる**ため、この 1 署名が abstract2.md §3.3.2 の
...
| `CloseIntent`(`channel.rs:630-725`) | [変] 同上の置換 + **(新)`final_state_version: u64`** と **(新)`final_settled_tx_chain: Bytes32`** を追加(L1 照合用)。`signing_digest`(IMCI)preimage に両者を追記。 |
| `WithdrawalClaim`(`channel.rs:729-765`) | [変] `user_amount: LatticeCommitment` → `user_amount_ct: RegevCiphertext`。`claim_proof` = `withdrawClaimZKP`(§E-3)。nullifier 導出([IMCW, close_intent_digest, user_id])は [維]。 |
| `PostCloseIncomingClaim`(`channel.rs:847-877`) | [変] `receiver_amount` を `RegevCiphertext` に。abstract2.md §3.5.5 `claimLateTx` の実装。`lateBalanceProof` は `claim_proof` 内で検証され、`finalBalanceProof` とは**別変数**(契約 storage 上も `usedSharedNativeNullifiers` 系で分離)で管理。 |
| `SpecialClose` / `CancelClose` | [維](abstract2.md の範囲外の追加防御。安全性質を弱めない追加なので存置。§I-3) |

### C-9. [維] base 層型

`Transfer`(`transfer.rs:34-39`、TRANSFER_LEN = 9)、`SettledTransfer`(nullifier 含む)、
`Block`、`PublicState`、`ValidityPublicInputs`、`ChannelId` / `KeyId` / `UserId` /
`KeyRecord` / `ChannelRecord` / `MemberSignature` — すべて変更なし。
abstract2.md の命名方針(base 層は既存実装名を使う)と一致。
...
`rangeProof`(abstract2.md §3.3.1)= この ZKP の**検証**(ITS = bp_key_id member が BP に渡す前に実施)。
...
- small block 署名(`channelStateSig` 相当 = `SignedSmallBlock.signatures` +
  `aggregated_signature_proof`)の検証に、**「署名 preimage の `tx_tree_root` 成分 = 当該
  small block の `tx_tree_root`」かつ「チャネル間経路では `tx_tree_root ≠ 0`」**の制約を追加。
- `PublicState.account_tree_root` の `ChannelLeaf.prev` 更新(取り込みブロック番号、二重支払い防止)は [維]。

### F-3. ChannelClosePublicInputs(`close_pis.rs:13-27`)
...
`CHANNEL_CLOSE_PUBLIC_INPUTS_LEN = 68 + 10 = 78`。
...
IMCR / IMKR / IMLD。木: `CHANNEL_TREE_HEIGHT = 32`, `KEY_TREE_HEIGHT = 32`,
`KEY_SET_TREE_HEIGHT = 3`(key_id あたり最大 8 SPHINCS+ 鍵), `MEMBER_KEY_TREE_HEIGHT = 8`,
...
| abstract2.md | 実装(更新版) |
|---|---|
| §3.0 `publishRegevPk` | channel 作成時、`registerChannel` の member 確定に合わせ `RegevPk` を channel 内配布。`memberKeys[channel_id]` は off-chain 管理 + `ChannelRecord` 拡張(`regev_pk_root: Bytes32` を `signing_digest` に追加) |
...
| §3.3.1 `rangeProof` | bp_key_id member が `channelUpdateZKP` を `RegevProofVerifier` で検証 |
