...
- **残: S6**（実 Sepolia 実行）= ユーザーの RPC + 資金付き deployer 鍵待ち。鍵は .claude/worktrees/.eth、中身を読まずローカルコマンドのみで扱う（CLAUDE.md「Secrets & Key Handling」）。
- 第2マイルストーン（送金 lifecycle: register→deposit→transfer→close を実チェーン駆動）は未着手。
