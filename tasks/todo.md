...
## 結果記録
- **S1-S5 完了**（commit f0fae92）: block_fixture emitter + MleFinalizeE2E フルパステスト + Deploy/Finalize script + blob runbook + .env/foundry.toml。anvil で deploy→postBlock(blob)→finalize 全路実証（実 MLE 検証 ON）。
- **EIP-170 解消**（submodule 2a1f5028 + parent ref bump）: MleVerifier 53,975→14,030B。`Plonky2GateEvaluator.evalCombinedFlat` と `SpongefishWhirVerify.verifyWhirProof` を internal→external 化（別 deploy + delegatecall link）。抽出 lib 21,911/22,218B も limit 内。**`--disable-code-size-limit` 無しの anvil deploy 成功＝実 EIP-170 下で deploy 可能**。振る舞い保存（submodule 79/79 incl. tamper、MleE2E/MleFinalizeE2E、finalize gas 11.81M→11.70M）。独立敵対レビュー = SOUND。runbook 更新済み。
- **S6 完了 ✅ 実 Sepolia smoke 完走（2026-06-14）**: deploy(MleVerifier 14,030B が実 EIP-170 受理) → postBlock(blob tx `0xb106e2d2…`, blockHashChainAt(1)=0x3ed4… 一致) → finalize(`0x766a6fa4…`, latestFinalizedStateRoot=0x2cfa6af8… 一致, **実 MLE/WHIR on-chain 検証成立**)。IntmaxRollup `0xBa05…0af0`, MleVerifier `0x4154…d871`。記録は docs/sepolia-smoke-runbook.md「Real Sepolia deployment record」。鍵は .claude/worktrees（中身読まずシェル展開のみ）。
- 第2マイルストーン（送金 lifecycle: register→deposit→transfer→close を実チェーン駆動、opus/codex 2メンバー）は未着手。fixture pipeline を deposit+off-chain transfer+close proof 列に拡張要（現 fixture は空ブロック1個）。
