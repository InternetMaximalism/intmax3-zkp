...
**Method:** Static review, supplementing the balance audit (`tasks/balance-audit-findings.md`).
...
`docs/spec.md` §7.4 describes the withdrawal circuit as:
...
The duplicate-insertion bug in `IndexedMerkleTree` (nullifier tree) documented in `tasks/balance-audit-findings.md` § BAL-CRIT-001 inflates a user's private balance via the `receive_transfer` / `receive_deposit` paths. That inflation flows naturally into withdrawals: the attacker spends the inflated balance through `send_tx_circuit`, then withdraws. No withdrawal-specific fix is needed — fix BAL-CRIT-001 to close this.
