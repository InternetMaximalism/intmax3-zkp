...
/-
  ...
  Source: `contracts/src/IntmaxRollup.sol` (`deposit` :815, `withdrawNative` :1307)

  The headline system-safety property: over ANY sequence of `deposit` /
  `withdrawNative` operations, the rollup never pays out more native ETH
  than was deposited. We prove the conservation law
  ...
  per-operation effects: `deposit` adds (checked), `withdrawNative`
  subtracts with the Solidity-0.8 underflow revert. The validity/MLE
  proofs gate WHICH withdrawals are allowed (and that they are
  circuit-backed) but are irrelevant to this accounting bound — solvency
...
/-- A native-fund operation: deposit `amount`, or `withdrawNative` paying
    leaves `ws` (proof preconditions are orthogonal to escrow accounting). -/
inductive Op where
  | dep (amount : U256)
  | wd  (ws : List Withdrawal)

/-- Escrow added by an op. -/
def depDelta : Op → U256 | .dep a => a | .wd _ => 0
/-- Escrow removed by an op. -/
def wdDelta : Op → U256 | .dep _ => 0 | .wd ws => totalAmount ws
...
/-- **Global solvency.** Total ETH withdrawn never exceeds initial escrow
    plus total ETH deposited. From a zero-escrow genesis this is exactly
    `Σ withdrawn ≤ Σ deposited`: the rollup cannot pay out more than was
    put in — unconditionally, independent of the proofs. -/
