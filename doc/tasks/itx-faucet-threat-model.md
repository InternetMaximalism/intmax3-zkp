...
| F-14 | A mistyped `FAUCET_DRIP` / `FAUCET_CHANNEL_CAP` hands out orders of magnitude more than intended | HARD ceilings `MAX_DRIP` (1,000 ITX per claim) and `MAX_CHANNEL_CAP` (10,000,000 ITX per channel), checked BEFORE the u64 bound and **not env-overridable** — crossing them needs a code change, and therefore review. u64 alone was never a policy bound: at 6 decimals it admits ~18.4 trillion ITX, so `FAUCET_DRIP=100000000000` (one extra zero-group) used to validate and drain the allowance in a single call | `faucetConfig` + tests |
