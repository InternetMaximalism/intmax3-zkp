...
仕様: `doc/tasks/a3-close-lifecycle-spec.md`(承認済み・全スコープ)
...
- [x] **P6-A specialClose / lateOutgoingDebit revert 化(完了・攻撃者レビュー GO)**: 両 entry を即 revert
  ...
  `0xED5e1c64…1A8FA8`、close_ fixture 再生成、CloseLifecycleE2E PASS。詳細 `doc/tasks/a3-p6a-stub-revert-plan.md`。
  dead-code 除去は deferred(detail2 §H-3 に明記)。
- [x] **fail-closed スタブ撤去・followup 更新(完了)**: `cmd_close_lifecycle_unimplemented` は P4 で撤去済。
  `doc/tasks/a3-close-lifecycle-followup.md` を「ほぼ完了」に更新。detail2 §H-3「IMPLEMENTED」、
