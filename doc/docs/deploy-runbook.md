...
| rollup **and** the channel close/claim lifecycle | `DeployCloseCli.s.sol` | everything above **plus** `ChannelSettlementVerifier`, `CloseFundingMaterializer`, and `ChannelSettlementManager`, with the four live settlement VKs (close, withdrawalClaim, postCloseClaim, cancelClose), plus `registerChannel` **and `registerSettlementManager`**. Direct member-set updates are retired: there is no MSU VK or verifier initializer. The production `channel_member deploy-settlement <rpc>` command stages `contracts/test/data/cli_reg_record.json`, pins the complete deployment intent before broadcast, runs this script, and validates its broadcast artifact and finalized read-back. |
...
**RETIRED 2026-08-31 — direct member-set update is not a deployable feature.** The active
`ChannelSettlementVerifier` contains no MSU VK, initializer, or verification function. The Manager
...
deposit → post → finalize → `withdrawNative`/`withdrawERC20`, and nothing else. Attach the channel
stack with `channel_member deploy-settlement <rpc>` from the channel work directory. Chain 31337
selects the explicitly local mock script; every other chain selects `DeployCloseCli.s.sol` and
refuses deterministic test cosigner keys. Before Forge broadcasts, the CLI fsyncs a PREPARED record
that pins the chain, broadcaster, starting nonce/block, script/fixture/source digest, rollup, and
signed participant identity. Recovery accepts only the same Foundry artifact: optional canonical
CREATE2 libraries followed by exactly ten core transactions, including the
`CloseFundingMaterializer` CREATE, Manager CREATE, and final Manager registration. It re-encodes
every CALL, verifies both constructor ABI suffixes from raw init-code, and rejects any sender,
nonce, value, target, argument, or ordering substitution.

The binding becomes ACTIVE only after the registration receipt is canonical and finalized and all
historical reads at that block agree: VK latches, runtime bytecode hashes for rollup/verifier/
manager/materializer, Manager→verifier/rollup/materializer links, materializer→rollup link,
channel/member/delegate identity, and Manager registration. `settlement.json` then includes
`close_funding_materializer`, the finalized activation checkpoint, and the four runtime-code
hashes. Every later close or fund-moving command revalidates the durable checkpoint, all four code
hashes, and both materializer links; a legacy binding without the helper fails closed.
...
second, which made guarded finalization reachable one block after a close intent was submitted and the
