...
Source snapshot: parent `05ec7ae94701f05d2aaf97ff796b7f800a6ce1f8`.
Updated 2026-09-06 from the earlier c533e71 projection. Source hashes and
function-level correspondence are recorded in the current review manifest.
...
* `Step.register` / `registerState`: `submitWithdrawalClaim`.
...
* `Step.fund` / `fundState`: `_pullChannelFunds`. The caller observes
  `materializedChannelExit(channelId) == finalizedCloseIntentDigest` and the
  EXACT balance delta `cap - received`. The old per-token IPW2 flags are retired.
* `Step.pay` / `payState`: `claimWithdrawalCredit(bytes32)`.
...
`FundingObservation` exposes the whole-state materialization identity and actual
balance-delta checks. Their provenance depends on
`CloseFundingMaterializer.materializeSignedHead` and the bound Rollup
`creditChannelExit` calls in its atomic whole-vector transaction. That binding,
...
inductive Step (cfg : Config) (proofAccepted : WithdrawalProofAccepted) :
  ...
  | fund {s : State} {t : Token} {obs : FundingObservation}
      ...
      (materialized : obs.materializedCloseId = cfg.closeId)
...
theorem step_preserves_safety {cfg proofAccepted s e s'}
  ...
  | fund token notFull materialized delta =>
    constructor
    · exact hs.accrualCap
    · intro t
...
theorem positive_fund_register_pay :
  ...
  apply Trace.cons (Step.fund (cfg := sampleConfig) (s := empty) (obs := ⟨3, 10⟩)
    (Or.inl rfl) (by decide) rfl (by decide))
...
NOT proved here. It projects these current Manager functions:

* requestClose/requestCloseAsParticipant/_requestClose;
* submitCloseIntent/_storePendingClose;
* cancelClose (there is NO cancel deadline guard);
* finalizeCloseGuarded/_finalizeClose;
* strict lexicographic _isNewer.
...
metadata, finalized Rollup root, bound full-verifier and backing receipt checks.
...
omitted checked cap aggregation and external tokenFundsDigest call. A false
tail rolls back the whole call. The current code does NOT splice a stale close
with a burn snapshot. It admits one whole state above both burn high-water
marks before storing the intent; ChannelSafetyExit models that separate rule.
Satellite freeze/thaw rejection and these high-water admission guards are
omitted here, so successful projected request/submit/cancel steps may still be
rejected by the implementation. Positive examples are not real-call liveness
proofs. Erasing such reverted calls preserves the replay-fence trace results;
an executable Rust/Solidity-to-model refinement is still not provided.
