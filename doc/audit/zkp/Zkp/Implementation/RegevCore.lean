...
* No lattice hardness. Secrecy of an amount given `(a, b, c1, c2)` is outside the
  model; the Ring-LWE assumption is a named boundary.
...
The `Zkp.Implementation.DecryptionGadget` import is used to cross-check the
constants and the amount encoding: that module models the in-circuit decryption
of exactly this scheme, so the parameter set, the digest word streams and the
64-bit little-endian message layout must agree, and the agreements are stated as
theorems.
-/
...
/-- The parameter set agrees with the one the in-circuit decryption gadget pins,
including the scaling factor `Δ` with `t · Δ + 1 = q`. -/
theorem constants_agree_with_decryption_gadget :
    ...
    DecryptionGadget.delta = 7864320 :=
...
theorem noise_and_digit_budget :
    ...
    DecryptionGadget.halfDelta = 3932160 :=
...
theorem domain_separators_distinct :
    ...
    regevPkRootDomain ≠ regevCtDomain ∧ regevPkPoseidonDomain ≠ regevCtDomain :=
  ⟨by decide, by decide, by decide, by decide, by decide, by decide⟩

/-! ## Generic list helpers (no counterpart in the source) -/

theorem replicate_succ_append (a : Nat) (n : Nat) :
    List.replicate (n + 1) a = List.replicate n a ++ [a] := by
  induction n with
  | zero => rfl
  | succ n ih => exact congrArg (fun l => a :: l) ih

theorem replicate_add_append (a : Nat) (m : Nat) :
    ∀ n, List.replicate (m + n) a = List.replicate m a ++ List.replicate n a := by
  induction m with
  | zero => intro n; simp
  | succ m ih =>
    intro n
    ...
    rw [hmn]
    exact congrArg (fun l => a :: l) (ih n)

theorem any_replicate_ne_zero (n : Nat) :
    ((List.replicate n (0 : Nat)).any (fun c => c != 0)) = false := by
  induction n with
  | zero => rfl
...
theorem map_const_range (n : Nat) (c : Nat) (f : Nat → Nat) (hf : ∀ i, f i = c) :
  ...
  | zero => rfl
  | succ n ih =>
    rw [range_succ_append, List.map_append, ih, replicate_succ_append]
    simp [hf n]

/-! ## Errors and data (`src/regev/encrypt.rs`, `src/regev/keys.rs`)

The Rust variants carry `String` payloads; the model keeps only the variant, so
error-message content is out of scope. -/
...
/-- The ternary-secret shape the source states for `channel_keygen` output. It is
a property of the upstream sampler, asserted here, not derived. -/
...
/-- `RegevPk::validate`: exact length first, then canonicality scanning `a` then `b`. -/
...
theorem pk_validate_dichotomy (pk : RegevPk) :
    pk.validate = .ok () ∨ pk.validate = .error .invalidPk := by
...
theorem ct_validate_dichotomy (ct : RegevCiphertext) :
    ct.validate = .ok () ∨ ct.validate = .error .invalidCiphertext := by
...
theorem pk_validate_ok_iff (pk : RegevPk) :
  ...
  constructor
  · intro h
    unfold RegevPk.validate at h
    split at h
    · next hl =>
      split at h
      · next hc => exact ⟨hl.1, hl.2, (coeffs_canonical_iff _).1 hc⟩
      · exact absurd h (by simp)
    · exact absurd h (by simp)
  · rintro ⟨h1, h2, h3⟩
    unfold RegevPk.validate
    rw [if_pos ⟨h1, h2⟩, if_pos ((coeffs_canonical_iff _).2 h3)]
...
theorem ct_validate_ok_iff (ct : RegevCiphertext) :
  ...
  constructor
  · intro h
    unfold RegevCiphertext.validate at h
    split at h
    · next hl =>
      split at h
      · next hc => exact ⟨hl.1, hl.2, (coeffs_canonical_iff _).1 hc⟩
...
theorem padding_ct_validates : RegevCiphertext.padding.validate = .ok () := by
  refine (ct_validate_ok_iff _).2 ⟨List.length_replicate _ _, List.length_replicate _ _, ?_⟩
  intro c hc
...
theorem padding_pk_validates : RegevPk.padding.validate = .ok () := by
  refine (pk_validate_ok_iff _).2 ⟨List.length_replicate _ _, List.length_replicate _ _, ?_⟩
  intro c hc
  rcases List.mem_append.1 hc with h | h <;> exact replicate_zero_canonical _ _ h

/-- `RegevCiphertext::validate_balance_exit_shape`: host acceptance of a newly
accepted balance. Canonical zero stays acceptable as an empty slot; zero `c1`
with nonzero `c2` is refused. This establishes no plaintext, range or
...
theorem exit_shape_accepts_padding :
  ...
  have h : (RegevCiphertext.padding.c2.any fun c => c != 0) = false :=
    any_replicate_ne_zero regevN
  simp [h]
...
theorem exit_shape_requires_canonical (ct : RegevCiphertext) (h : ct.validate ≠ .ok ()) :
    ct.validateBalanceExitShape = .error .invalidCiphertext := by
  rcases ct_validate_dichotomy ct with hv | hv
  · exact absurd hv h
  · simp [RegevCiphertext.validateBalanceExitShape, hv]
...
theorem pk_digest_none_on_invalid (pk : RegevPk) (h : pk.validate ≠ .ok ()) :
    pk.digestWords = none := by
  rcases pk_validate_dichotomy pk with hv | hv
  · exact absurd hv h
  · simp [RegevPk.digestWords, hv]

theorem pk_poseidon_none_on_invalid (pk : RegevPk) (h : pk.validate ≠ .ok ()) :
    pk.poseidonWords = none := by
  rcases pk_validate_dichotomy pk with hv | hv
  · exact absurd hv h
  · simp [RegevPk.poseidonWords, hv]

theorem ct_digest_none_on_invalid (ct : RegevCiphertext) (h : ct.validate ≠ .ok ()) :
    ct.digestWords = none := by
  rcases ct_validate_dichotomy ct with hv | hv
  · exact absurd hv h
  · simp [RegevCiphertext.digestWords, hv]
...
theorem digest_preimage_lengths (pk : RegevPk) (ct : RegevCiphertext)
  ...
  refine ⟨by simp [RegevPk.digestWords, hp], ?_, by simp [ha, hb, regevN],
    by simp [h1, h2, regevN]⟩
...
theorem pk_root_preimage_order_sensitive (digestOf : RegevPk → List Nat)
  ...
  intro heq
  unfold pkRootWords at heq
  have h : digestOf x ++ digestOf y = digestOf y ++ digestOf x := by
    simpa [List.bind] using heq
  exact hne (List.append_inj_left h (by rw [hx, hy]))

/-- The root preimage also binds the member count. -/
...
theorem bit_at_lt_two (v i : Nat) : bitAt v i < 2 := Nat.mod_lt _ (by decide)
...
theorem list_weighted_append (xs : List Nat) :
  ...
  | cons d ds ih =>
    ...
    simp only [List.cons_append, List.append_eq, listWeighted, List.length_cons,
      ih ys (i + 1), hidx]
    omega
...
theorem list_weighted_zip_add (xs : List Nat) :
  ...
  | cons d ds ih =>
    ...
    | cons e es =>
      ...
      have hd : (d + e) * 2 ^ i = d * 2 ^ i + e * 2 ^ i := by rw [Nat.add_mul]
      omega
...
theorem encode_amount_length (v : Nat) : (encodeAmount v).length = regevN := by
  unfold encodeAmount
  rw [List.length_append, List.length_map, range_length_eq, List.length_replicate]
  decide

/-- The coefficients above the 64 amount bits are zero, as the D1 encoding
requires ("the remaining coefficients stay zero in fresh encryptions"). -/
theorem encode_amount_high_coefficients_zero (v i : Nat)
    (h1 : amountBits ≤ i) (h2 : i < regevN) : (encodeAmount v)[i]? = some 0 := by
  unfold encodeAmount
  have hlen : ((List.range amountBits).map (bitAt v)).length = amountBits := by
    rw [List.length_map, range_length_eq]
  rw [List.getElem?_append_right (by rw [hlen]; exact h1), hlen, List.getElem?_replicate]
  have hi : i - amountBits < regevN - amountBits := by
    simp only [amountBits, regevN] at *
    omega
  simp [hi]
...
theorem encode_amount_matches_decryption_gadget (v : Nat) :
  ...
  have hbit : ∀ i : Nat, bitAt v i = DecryptionGadget.boolNat (decide (v / 2 ^ i % 2 = 1)) := by
    intro i
    have h : bitAt v i < 2 := bit_at_lt_two v i
    unfold bitAt at h ⊢
    by_cases hb : v / 2 ^ i % 2 = 1
    · rw [hb]; simp [DecryptionGadget.boolNat]
    · have h0 : v / 2 ^ i % 2 = 0 := by omega
      rw [h0]; simp [DecryptionGadget.boolNat]
  simp only [encodeAmount, DecryptionGadget.encodeAmount, DecryptionGadget.bitsOf,
    ...
    DecryptionGadget.ringN, Function.comp]
  congr 1
  exact List.map_congr_left (fun i _ => hbit i)
...
theorem encode_amount_zero : encodeAmount 0 = List.replicate regevN 0 := by
  ...
  unfold encodeAmount
  rw [h]
  have hsplit : regevN = amountBits + (regevN - amountBits) := by decide
  rw [hsplit]
  exact (replicate_add_append 0 amountBits (regevN - amountBits)).symm
...
theorem decode_loop_prefix (xs : List Nat) :
  ...
  | cons d ds ih =>
    ...
    simp only [List.cons_append, hstep, ih rest (i + 1) (value + d * 2 ^ i) (by omega),
      listWeighted, List.length_cons, hidx, Nat.add_assoc]
...
theorem decode_loop_zero_prefix (m : Nat) :
  ...
  | succ m ih =>
    intro rest i value
    have hstep : decodeLoop (0 :: (List.replicate m 0 ++ rest)) i value
        = decodeLoop (List.replicate m 0 ++ rest) (i + 1) value := by simp [decodeLoop]
    have hidx : i + 1 + m = i + (m + 1) := by omega
    simp only [List.replicate, List.cons_append, hstep, ih rest (i + 1) value, hidx]

theorem decode_loop_zeros (m : Nat) (i value : Nat) :
    decodeLoop (List.replicate m 0) i value
      = decodeLoop ([] : List Nat) (i + m) value := by
  have h := decode_loop_zero_prefix m [] i value
  simpa using h
...
theorem decode_padded (xs : List Nat) (m : Nat) (hlen : xs.length ≤ 64)
  ...
  rw [decode_loop_prefix xs (List.replicate m 0) 0 0 (by omega),
    decode_loop_zeros m (0 + xs.length) (0 + listWeighted xs 0)]
  simp only [decodeLoop, Nat.zero_add]
  simp [hv]

/-- A nonzero digit at weight `2 ^ 64` or above is rejected: the source returns
`DecryptOverflow` here instead of letting the upstream decoder panic on an
adversarially crafted ciphertext. -/
...
theorem decode_encode_roundtrip (v : Nat) (h : v < 2 ^ 64) :
  ...
  have hlen : ((List.range amountBits).map (bitAt v)).length ≤ 64 := by
    rw [List.length_map, range_length_eq]
    decide
  have hd := decode_padded ((List.range amountBits).map (bitAt v)) (regevN - amountBits)
...
theorem zip_add_replicate_zero (m : Nat) :
    List.zipWith (fun a b => a + b) (List.replicate m (0 : Nat)) (List.replicate m 0)
...
theorem add_digits_of_encodings (a b : Nat) :
  ...
  rw [List.zipWith_append _ _ _ _ _ (by rw [List.length_map, List.length_map]),
    zip_add_replicate_zero]

/-- Digit-wise addition of two encoded amounts decodes to the sum: the plaintext
level of the `add_ciphertexts` claim, and the reason the digit headroom `t = 256`
bounds the number of stacked additions. -/
theorem encoded_digit_sum_decodes_to_sum (a b : Nat) (ha : a < 2 ^ 64)
  ...
  have hla : ((List.range amountBits).map (bitAt a)).length
      = ((List.range amountBits).map (bitAt b)).length := by
  ...
  have hlen : (List.zipWith (fun x y => x + y) ((List.range amountBits).map (bitAt a))
    ...
    rw [List.length_zipWith, List.length_map, List.length_map, range_length_eq]
    simp [amountBits]
  rw [add_digits_of_encodings a b,
    decode_padded _ (regevN - amountBits) hlen (by rw [hsum]; exact hab), hsum]
...
taken coefficient-wise with an explicit reduction mod `q`, so the result is
always the canonical representative. -/
def addCiphertexts (a b : RegevCiphertext) : Except RegevError RegevCiphertext :=
  match a.validate with
  | .error e => .error e
  | .ok _ =>
      match b.validate with
      | .error e => .error e
      | .ok _ => .ok ⟨addCoeffs a.c1 b.c1, addCoeffs a.c2 b.c2⟩
...
theorem add_ciphertexts_checks_left_operand_first (a b : RegevCiphertext)
  ...
  rcases ct_validate_dichotomy a with hv | hv
  · exact absurd hv h
  · simp [addCiphertexts, hv]
...
theorem add_ciphertexts_rejects_noncanonical_right (a b : RegevCiphertext)
  ...
  rcases ct_validate_dichotomy b with hv | hv
  · exact absurd hv h
  · simp [addCiphertexts, ha, hv]

theorem add_coeffs_canonical (x : List Nat) :
    ∀ (y : List Nat) (c : Nat), c ∈ addCoeffs x y → c < regevQ := by
  induction x with
  | nil => intro y c hc; simp [addCoeffs] at hc
  | cons a as ih =>
    intro y c hc
    cases y with
    | nil => simp [addCoeffs] at hc
    | cons b bs =>
      simp only [addCoeffs, List.zipWith_cons_cons, List.mem_cons] at hc
      rcases hc with h | h
...
theorem add_coeffs_length (x y : List Nat) (hx : x.length = regevN)
  ...
  rw [addCoeffs, List.length_zipWith, hx, hy, Nat.min_self]
...
theorem add_ciphertexts_result_validates (a b c : RegevCiphertext)
  ...
  rcases ct_validate_dichotomy a with ha | ha
  · rcases ct_validate_dichotomy b with hb | hb
    · simp only [addCiphertexts, ha, hb] at h
      obtain ⟨ha1, ha2, _⟩ := (ct_validate_ok_iff a).1 ha
      obtain ⟨hb1, hb2, _⟩ := (ct_validate_ok_iff b).1 hb
      rw [← Except.ok.inj h]
      refine (ct_validate_ok_iff _).2 ⟨add_coeffs_length _ _ ha1 hb1,
        add_coeffs_length _ _ ha2 hb2, ?_⟩
      intro d hd
      rcases List.mem_append.1 hd with h' | h'
      · exact add_coeffs_canonical _ _ _ h'
      · exact add_coeffs_canonical _ _ _ h'
    · simp [addCiphertexts, ha, hb] at h
  · simp [addCiphertexts, ha] at h
...
theorem add_coeffs_zero_left (ys : List Nat) :
  ...
  | cons y ys ih =>
    ...
    | succ m =>
      ...
      have hrest : ∀ c ∈ ys, c < regevQ := fun c h => hc c (List.mem_cons.2 (Or.inr h))
      have htail := ih m hlen hrest
      unfold addCoeffs at htail ⊢
      simp only [List.replicate, List.zipWith_cons_cons, Nat.zero_add, htail,
        Nat.mod_eq_of_lt hy]
...
theorem add_ciphertexts_padding_identity (ct : RegevCiphertext)
  ...
  simp only [addCiphertexts, padding_ct_validates, h, e1, e2]
...
Sender-private in the source (deliberately not `Serialize`). -/
...
randomness satisfies the ternary / CBD-half range predicates the witness check
applies. It claims `amount = 0` and no other amount. -/
...
def encryptAmount (encryptOracle : RegevPk → List Nat → RegevCiphertext × EncryptionRandomness)
    ...
    Except RegevError (RegevCiphertext × AmountWitness) :=
  match pk.validate with
  | .error e => .error e
  | .ok _ =>
      let message := encodeAmount amount
      let out := encryptOracle pk message
      .ok (out.1, ⟨amount, message, out.2⟩)

theorem encrypt_amount_witness_message
    (encryptOracle : RegevPk → List Nat → RegevCiphertext × EncryptionRandomness)
    (pk : RegevPk) (amount : Nat) (ct : RegevCiphertext) (w : AmountWitness)
    (h : encryptAmount encryptOracle pk amount = .ok (ct, w)) :
    w.amount = amount ∧ w.m = encodeAmount amount := by
  rcases pk_validate_dichotomy pk with hv | hv
  · simp only [encryptAmount, hv] at h
    have hw : w = ⟨amount, encodeAmount amount, (encryptOracle pk (encodeAmount amount)).2⟩ :=
      (congrArg Prod.snd (Except.ok.inj h)).symm
    rw [hw]
    exact ⟨rfl, rfl⟩
  · simp [encryptAmount, hv] at h
...
theorem encrypt_amount_refuses_before_oracle
    (o1 o2 : RegevPk → List Nat → RegevCiphertext × EncryptionRandomness)
    (pk : RegevPk) (amount : Nat) (h : pk.validate ≠ .ok ()) :
    encryptAmount o1 pk amount = .error .invalidPk ∧
    encryptAmount o2 pk amount = .error .invalidPk := by
  rcases pk_validate_dichotomy pk with hv | hv
  · exact absurd hv h
  · exact ⟨by simp [encryptAmount, hv], by simp [encryptAmount, hv]⟩
...
def decryptAmount (digitOracle : RegevSk → RegevCiphertext → List Nat)
    (sk : RegevSk) (ct : RegevCiphertext) : Except RegevError Nat :=
  if sk.s.length = regevN then
    match ct.validate with
    | .error e => .error e
    | .ok _ => decodeValue (digitOracle sk ct)
  else .error .invalidSk

theorem decrypt_amount_checks_secret_key_first
    (o1 o2 : RegevSk → RegevCiphertext → List Nat) (sk : RegevSk)
    (ct : RegevCiphertext) (h : sk.s.length ≠ regevN) :
    decryptAmount o1 sk ct = .error .invalidSk ∧
    decryptAmount o2 sk ct = .error .invalidSk := by
  exact ⟨by simp [decryptAmount, h], by simp [decryptAmount, h]⟩

/-- A non-canonical ciphertext never reaches the upstream decryptor. -/
theorem decrypt_amount_refuses_noncanonical_before_oracle
    (o1 o2 : RegevSk → RegevCiphertext → List Nat) (sk : RegevSk)
  ...
  rcases ct_validate_dichotomy ct with hv | hv
  · exact absurd hv h
  · exact ⟨by simp [decryptAmount, hs, hv], by simp [decryptAmount, hs, hv]⟩

/-! ### The two undischarged cryptographic premises -/

/-- "The oracle decrypts this ciphertext to these digits." Holds in the source
only within the noise budget and only for the matching key; it is never derived
here. -/
...
theorem homomorphic_pending_balance (digitOracle : RegevSk → RegevCiphertext → List Nat)
    (sk : RegevSk) (x y z : RegevCiphertext) (a b : Nat) (hs : sk.s.length = regevN)
  ...
  simp only [decryptAmount, hs, hz, if_pos, hhom x y z hadd, hx, hy]
  exact encoded_digit_sum_decodes_to_sum a b ha hb hab

/-! ## Concrete normal traces -/
...
theorem normal_padding_trace (ct : RegevCiphertext) (h : ct.validate = .ok ()) :
    ...
    addCiphertexts RegevCiphertext.padding ct = .ok ct :=
  ⟨padding_ct_validates, exit_shape_accepts_padding,
