...
uses its deployment chain, not a universal 31337 restriction. That surrounding
modifier and materializer escrow accounting are outside this projection.
...
* contracts/src/IntmaxRollup.sol: finalize, fraudProof, fullVerify,
  _verifyFraud and their DA/state/height/rollback guards.
* contracts/lib/polygon-plonky2/mle/contracts/src/PinnedMleVerifierV2.sol:
  constructor/config, verifyCompactPublicInputs, fraudVerdictCompact,
  compactFraudVerdictBody; CompactMleProofV2.sol decoder.
* src/utils/mle_prover.rs and the pinned Rust mle/src/verifier_v2.rs
  (protocol/codec/core boundaries, not a proof of their cryptographic semantics).
...
inductive Purpose where
  | validity | withdrawal | close | withdrawalClaim | postCloseClaim | cancelClose | closeBacking
