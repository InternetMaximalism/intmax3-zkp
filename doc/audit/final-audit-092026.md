...
### 2. 現在のコードに、重大（critical）な脆弱性は見つかっていない
...
### 3. これまでに見つかった脆弱性は修正されている

監査の過程で複数の実際に悪用可能な欠陥が見つかり、**そのすべてが修正され、再発防止の
回帰テストが追加されている**（第4節に一覧）。リリースを止めていた 3 件の判定
（NO-GO）もすべて解消した。

ただし正確を期すと、**深刻度が「高」以下の未解決項目が 3 件残っている**（第5節）。
いずれも単独では悪用できず、修正方針も確定している。これらを「脆弱性はゼロ」と表現する
ことはしない。
...
## 5. 現在も追跡している項目（重大なものはない）

正直に記す。以下は未解決だが、いずれも重大には該当しない。
...
- **未解決の追跡項目が 3 件あり、最も重いものでも「高」で、修正方針は確定している。**
- **リリース可の判定には、外部レビューと本番環境での通し確認が必要である。**
