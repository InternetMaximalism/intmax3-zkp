...
- **残タスク**: CLI `channel_member` の `send` / `gen-send` / inter / burn コマンドは **まだ witnessed 経路**（seed から witness 再構成、relay の faucet は `refresh`→`send`）。復号版へ切替えると witness-store（Genesis/Refreshed seed）と `invalidate_witness` が不要になる。実ブラウザでの通し確認（WASM 再ビルド `hosting/build-wallet-wasm.sh` 後）。
