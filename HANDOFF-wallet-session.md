...
- **実ブラウザ通し確認（2026-09-21, 組込みブラウザ）**: relay＋anvil＋daemon を起動し、ブラウザと同じ seed 導出（`wallet_keygen_seeded`）で node 版 WASM から delegate join（7-3）→ 運営者入金 `/api/l1-deposit` → `/api/import-deposit`（準同型加算、pendingAdds=1）。ブラウザで localStorage に seed を入れて復元 → **0.005 ETH を 7-0 へ送金成功（7 秒、refresh ステップ無し、relay 側も `cosign` のみ、送信者 pending_adds リセット）**。手順スクリプト: `scratchpad/headless-join-deposit.js`（相当のものは node/common/wallet.js で数行）。
- **inter-channel（ブラウザ）: 通し確認完了（2026-09-21）**。7-3 → 8-0 へ 0.004 / 0.003 / 0.002 ETH（さらに再 bootstrap 後に 0.004、47 秒）を送金し、復号版 E-2 の debit 証明を relay が co-sign、宛先 channel 8 の two-state credit gate が受理（ch8 は v6→、fund 0.019、slot 0 に準同型 credit ×3）。最後の1回はページ修正込みで「sent 0.002 ETH to 8-0 (79s)」まで完走。
- **ローカル2チャンネル stack の修正（今回）**: (1) `wallet-relay.js` bootstrap は **rollup を1つ共有**し、backing 入金は bootstrap 順に即 journal（`api/lib/deposit-pipeline.js` の `journalBackingDeposit`）。共有 producer は L1 deposit index を単一列で要求するため、チャンネル毎の rollup（index が 0 から再開）では2本目が永遠に adoption 不能だった。(2) `setup-backing` は fresh な witness generator で genesis を証明する＝producer の「最初のブロック」にしかなれないので、**2本目以降は `SETUP_BACKING_EMPTY_GENESIS=1`（入金なし・fund 0・settle chain 0）**で作り、`init` は backing fund 0 のとき cosigner 残高を 0 にする（Σ genesis == fund）。空 genesis の live balance は legacy relay の `/api/init` が `liveInitWithAccountSalt`＋bind＋register で開始（`api/routes/channel-init.js` と同じ。旧 legacy 経路は何もせず、初回 import が "balance.snapshot does not exist" になっていた。pipeline の `ensureLiveBackingAdopted` は未入金 genesis を「adoption 不要」として早期 return のまま＝node テストが固定する挙動）。(3) `setup-backing` の keystone は rollup の直前 `Deposited` ログの `newDepositHashChain` を prev として照合（共有 rollup では 0 ではない）。(4) `wallet-live.html` の inter-channel finalize は relay の `sourceHead` を読む（`aHead` は旧名で undefined → "reading 'length'" で失敗していた）。
- **注意（運用）**: `block_producer_service` バイナリは `cargo build --release --bin block_producer_service` で明示的に再ビルドすること（古いバイナリが残っていると宛先の credit gate が旧 verifier で `InvalidProofShape` を返す）。資金投入の `cast send` は anvil アカウント #1 から行う（#0 は forge のデプロイと nonce 競合）。
- **既存失敗の修正**: `tests/inter_channel_unified_e2e.rs` は初回送金の nonce を 0（channel leaf の index）に修正して緑（`0d21e5c`）。`tests/inter_channel_cli.rs` の残り2件（credit 側 signer exit-kit receipt 必須）は、フィクスチャに検証済み archive を要するため未修正。
- **クラスタ co-sign プロトコル（2026-09-21, 仕様: doc/tasks/cluster-cosign-protocol.md）**: N≤8 ホストが各自のスロットで `cosign-partial`（ゲート＋自分の署名、head 未進行）→ 全員へ署名を N-to-N 送信 → 全署名を持つホストが `cosign-merge`（署名を個別検証して N-of-N 採用）。1分で欠落スロットを全員へ CLOSE 警告（毎分再送）、保持者（本人以外でも）が警告者へ供給、5分で HALT（`cluster_halt.json`、mutating 経路は 503、遅延到着で解除）、HALT 24時間で最終 signed state を `close`（watchdog、失敗は再試行）。実装: `api/lib/cluster.js`（transport/clock 注入、`node/test/cluster-cosign.test.js` 9件）、relay 配線 `INTMAX_CLUSTER_SELF_URL` / `INTMAX_CLUSTER_PEERS` / `INTMAX_CLUSTER_SIGN_SLOTS` / `INTMAX_CLUSTER_{WARN,HALT,CLOSE}_MS`、CLI `cosign-partial` / `cosign-merge`、`wallet_core::verify_member_signature`。self-only クラスタで実 stack を通し確認（2 秒で N-of-N）。**未対応**: refresh / inter-channel / deposit import / token 登録 / MSU の co-sign は単一ホスト経路のまま（同じ gate→partial→merge 分割で拡張可）。複数ホストの実機テスト（別ホストに同一 channel を配布）は未実施。
- **inter-channel の送り主離脱耐性（2026-09-21）**: source レグ commit 後に送り主が消えても relay が完遂する。`api/lib/inter-channel-send.js` の `pendingInterTransfer` / `resumePendingInterTransfer`、relay の `sweepPendingInterTransfers`（起動時＋`INTMAX_INTER_RESUME_MS`、既定 30 秒）、`/api/inter/send` は新規送金の前に pending を先に完了、`GET /api/inter/pending`、`POST /api/exit-kit/install`。実 stack で `INTMAX_TEST_FAIL_INTER_TRANSFER_AFTER_SOURCE=1` により 9→10 を途中離脱させ、failpoint 無しで再起動 → sweep が自動完遂（9 v3 0.005 / 10 v2 0.005、pending:false）。ついでに (1) 空 genesis チャンネルは初回 credit 前に exit-kit receipt が必要 → `/api/init` の空 genesis 分岐で `installHeadExitKit` を追加、(2) `build_inter_channel_credit` の `fund_import_state.h2_tag` を 0 にリセット（InvalidH2Tag）。**未解決（base 層の仕様、要設計判断）: receive-after-send window** — 一度 debit（send ブロック S）したチャンネルは「次の send ブロックの直前」でしか receive/deposit を証明できず（`receive_transfer_circuit`: `new_block_r < send_leaf.cur`）、現行 daemon は次の send をブロック直後に証明するので窓が開かない。ローカルの 8→7 はこれで pending のまま（`ensure_receive_window_open` が明示メッセージで拒否、sweep は再試行し続ける）。詳細と修正案（受信の遅延キュー＝intmax2 流）は `doc/tasks/inter-channel-live.md` 末尾。
- **API 全般レビュー（2026-09-22、3 系統: relay ルート / api/routes / api/lib）— 修正済み**: (1) クラスタ: ゲートで拒否された提案が round を開いたまま → 5 分で HALT・24h で自動 close・halt が relay をクラッシュ → 即時破棄に修正、halt タイマーも例外ガード。(2) relay `reqChannel` が不明チャネルを黙って ch7 に丸めていた（`?channel=8x` の join/cosign/import が 7 に落ちる）→ 400。`/api/inter/send` の宛先も CHANNELS 検証。(3) `/api/close` `/api/settle` `/api/withdraw` `/api/l1-deposit` `/api/add-genesis-sig` がロック無しで CLI 実行 → `withLock`。(4) batch 拒否時の solo 再生が `await` 無しで並列実行（payload.json 上書き・ロック早期解放）→ 逐次、head が進んだ後の payload は staleAnchor 409。(5) faucet / refresh-cosign が head を進めるのに `flushPublishedHead` を呼ばず、次の cosign で bind 拒否 → 追加。(6) inter-channel: CLI が commit 前に拒否（残高不足・stale head 等）すると `inter_operation.json` が prepared のまま残り、以後その source からの別送金が永久 409 + sweep が同じ失敗を繰り返し、staged exit-kit ブロックが全チャネルの producer 変更を凍結 → journal 不在なら abandon（`liveAbandonPreparedExitKit`）して 409。burn ルート（api/routes/burn.js, partial-withdrawal.js）も同様（ticket も削除）。(7) HALT 免除に `/api/exit-kit/install` と `/api/browser-claim/*`。(8) relay のエラー JSON から INSECURE KEYS バナーと重複行を除去、`e.status` を尊重。**未修正（報告のみ）**: `flushLastProducerBlock` が前回送金の宛先（第三のチャネル）をロック無しで変更する（inter-channel-send.js:178）; 宛先 recovery サイドカーが全成功後にしか書かれず、途中クラッシュで宛先の `/snapshot` が 404 化（producer-head.js recoverIncomingHead）; block-producer.js の exit/line ハンドラが子プロセスに紐付かず、クラッシュ時に別リクエストへ応答が混線しうる、`execute` にタイムアウト無し; api の `GET /snapshot` `/backing` が無認証で回復系ミューテーションを実行; `/api/cosign-burn` は 503 スタブで partial-withdrawal UI は動かない; `GET /status` は常に active; `/keys/generate` は operator 自身の slot 鍵を返す; API-DESIGN.md のドリフト多数（A8/A11/A26/A30/A33/W10、未記載ルート）。
