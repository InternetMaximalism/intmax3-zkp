...
- **inter-channel（ブラウザ）**: 復号版 E-2 の debit 証明生成と relay の `/api/inter/send` 受理まで確認。宛先 channel 8 側で「live balance snapshot が無い（initialize が必要）」で停止 → channel 8 へ入金して bind しようとすると「L1 deposit index 0 is stale; producer expects 2」（共有 producer に対する ch8 の backing deposit index 不整合＝既存の複数チャンネル bootstrap 順序問題、本変更と無関係）。その後は ch7 に pending の inter-channel 遷移が残り再試行不可（`recover-inter-transfers` の領域）。**宛先チャンネルの live balance を正しく bind した環境で再検証が必要**。
- **既存失敗（回帰ではない）**: `tests/inter_channel_unified_e2e.rs` は「tx_v2 nonce at i 0 must equal the previous channel send index (expected 0, got 1)」で失敗。ベース `e5bbd47` でも同一失敗を確認（テストが `base_nonce: 1` を固定で渡す vs ブロック証跡生成器の検査）。`tests/channel_backing_e2e.rs`（anvil、CLI send 経由）は緑。
- **残タスク**: 上記 inter-channel のブラウザ再検証（宛先 bind 済み環境）。anvil 必須の e2e（`itx_faucet_cli_e2e` は `#[ignore]`）は個別実行が必要。`tests/inter_channel_cli.rs` はフィクスチャ修正で 9→15/17（残り2件は credit 側の signer exit-kit receipt 必須化に対するフィクスチャ拡張が必要）。
