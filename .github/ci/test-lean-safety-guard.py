...
class LeanGuardTests(unittest.TestCase):
    def setUp(self):
        ...
        for project, modules in GUARD.CURRENT.items():
    ...
    def test_complete_manifest_is_accepted(self):
        self.assertEqual(len(self.validate()), sum(map(len, GUARD.CURRENT.values())))

    def test_duplicate_module_cannot_replace_missing_module(self):
        self.manifest["theorem_checks"][1] = copy.deepcopy(self.manifest["theorem_checks"][0])
        with self.assertRaisesRegex(GUARD.GuardFailure, "duplicate, or unknown current theorem module"):
            self.validate()

    def test_named_current_modules_in_one_project_are_all_audited(self):
        self.assertGreater(len(GUARD.CURRENT["doc/architecture-audit"]), 1)
        checks = self.validate()
        self.assertEqual({(c["project"], c["module"]) for c in checks},
                         {(p, m) for p, modules in GUARD.CURRENT.items() for m in modules})
    ...
    def test_unsupported_theorem_names_are_not_silently_omitted(self):
        self.assertEqual(GUARD.declared_theorems("theorem ordinary_name : True := True.intro"), ["ordinary_name"])
        for source in ("theorem 安全 : True := True.intro",
                       "theorem «quoted name» : True := True.intro",
                       "theorem Nested.name : True := True.intro"):
            with self.assertRaisesRegex(GUARD.GuardFailure, "unsupported theorem declaration syntax"):
                GUARD.declared_theorems(source)
    ...
    def test_native_decide_is_not_a_kernel_checked_shortcut(self):
        ...
        with patch.object(GUARD, "module_sources", return_value=modules):
            with self.assertRaisesRegex(GUARD.GuardFailure, "explicit admission/axiom"):
                GUARD.check_coverage(self.root)
