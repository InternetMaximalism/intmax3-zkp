...
models retain their documented trusted base; only the listed current modules get the
...
CURRENT = {
    ...
    "doc/audit/zkp": (
        ...
        "Zkp.Implementation.BalanceWitnesses",
        "Zkp.Implementation.BlockMessages",
        "Zkp.Implementation.BlockStep",
        "Zkp.Implementation.ChannelRegChain",
        "Zkp.Implementation.CrateLayout",
        "Zkp.Implementation.DepositChain",
        "Zkp.Implementation.FlowHarness",
        "Zkp.Implementation.Processors",
        "Zkp.Implementation.ReceiveDepositCircuit",
        "Zkp.Implementation.ReceiveTransferCircuit",
        "Zkp.Implementation.SendTxCircuit",
        "Zkp.Implementation.SignatureReleaseLedger",
        "Zkp.Implementation.SingleWithdrawalCircuit",
        "Zkp.Implementation.SystemSafety",
        "Zkp.Implementation.TrustBoundary",
        "Zkp.Implementation.TxSettlement",
        "Zkp.Implementation.UpdateChannelTree",
        "Zkp.Implementation.ValidityChain",
...
ARCH_ROOTS = frozenset({
    ...
    "ChannelSafetyCurrent", "ChannelSafetyAdmission", "ChannelSafetyRecovery", "ChannelSafetyExit",
})
...
def declared_theorems(source):
    # Deliberately support the ordinary, unqualified ASCII declarations used by
    # these modules. Reject other theorem-name syntax rather than silently omit
    # it. This is not a general Lean macro/environment declaration inventory.
    clean = without_comments_and_strings(source)
    names = re.findall(r"\btheorem\s+([^\s]+)", clean)
    require(len(names) == len(re.findall(r"\btheorem\b", clean))
            and all(re.fullmatch(IDENT, name) for name in names),
            "unsupported theorem declaration syntax; extend the reviewed inventory before use")
    return names
...
def check_coverage(root):
    ...
    for project, current_modules in CURRENT.items():
        ...
        require(set(current_modules) <= set(modules),
                f"current modules missing: {sorted(set(current_modules) - set(modules))}")
...
def validate_manifest(root, manifest):
    ...
    expected = {(project, module) for project, modules in CURRENT.items() for module in modules}
    require(isinstance(checks, list) and len(checks) == len(expected), "expected every current theorem module")
    checked_modules = set()
    for check in checks:
        exact_keys(check, {"project", "module", "theorems", "sources"}, "theorem check")
        project, module = check["project"], check["module"]
        require(isinstance(project, str) and isinstance(module, str)
                and (project, module) in expected and (project, module) not in checked_modules,
                "missing, duplicate, or unknown current theorem module")
        ...
        declarations = declared_theorems((root / model).read_text())
        ...
        checked_modules.add((project, module))
    require(checked_modules == expected, "current theorem module coverage is incomplete")
