...
///
/// Unlike the sequential SigAgg circuit, this circuit does NOT update the user tree.
...
///   last_user_id:                1
///   Total: 37
pub const SIG_BATCH_PUBLIC_INPUTS_LEN: usize = 3 * POSEIDON_HASH_OUT_LEN + 2 * BYTES32_LEN + 9;
...
{
    /// Read-only user tree root (snapshot at block start).
    pub account_tree_root: PoseidonHashOut,
    pub block_number: BlockNumber,
    pub channel_id: u32,
    pub tx_tree_root: Bytes32,
    /// IMSB `SmallBlockRootMessage::signing_digest()` every member signature in this batch is
    ...
    /// Cumulative hash of verified users: Poseidon(prev || channel_id).
    pub verified_users_hash: PoseidonHashOut,
    pub verified_count: u32,
    /// First finalized channel_id in this batch (0 if empty).
    pub first_user_id: u64,
    /// Last finalized channel_id in this batch (0 if empty).
