...
use crate::circuits::balance::{
    ...
    common::recipient::{calculate_recipient_from_address, calculate_recipient_from_user_id},
...
pub async fn run_single_withdrawal_proof() -> Result<(), JsValue> {
    ...
    let withdrawal_processor =
        ...
        .await;
    log_duration(
        "Withdrawal processor construction",
        (js_sys::Date::now() - t) / 1000.0,
    );
