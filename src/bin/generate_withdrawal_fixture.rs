...
//!   - WD_OUT_PREFIX=<prefix>     — filename prefix so a variant set does not overwrite the
//!     default. `close_` is REFUSED: the close family is co-generated by
//!     `generate_close_fixture` (lifecycle + close intent + CloseAssetBacking from one run).
//!
...
/// Shared with `generate_close_fixture` (`CLOSE_FIXTURE_DEPOSIT` / `CLOSE_FIXTURE_WITHDRAWAL`):
/// deposit 6, withdraw 3, so the plain and `close_` lifecycles differ only in the baked recipient.
const DEFAULT_DEPOSIT_AMOUNT: u64 = 6;
const DEFAULT_WITHDRAWAL_AMOUNT: u64 = 3;

/// The `close_` family is NOT produced here any more: `generate_close_fixture` co-generates the
/// whole close family (lifecycle + close intent + CloseAssetBacking) from ONE lifecycle so the
/// finalized state root, settled-tx chain and fund vector agree by construction. Refuse the old
/// prefix loudly rather than write a `close_` set the close/backing proofs do not bind.
fn refuse_close_family_prefix(prefix: &str) -> anyhow::Result<()> {
    if prefix == "close_" {
        anyhow::bail!(
            "WD_OUT_PREFIX=close_ is no longer supported: run \
             `WD_RECIPIENT=<manager> cargo run --release --features close-fixture-bin --bin \
             generate_close_fixture`, which writes the close_ lifecycle together with the close \
             intent and CloseAssetBacking artifacts"
        );
    }
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    ...
    refuse_close_family_prefix(&prefix)?;
    ...
    let params = ChannelWithdrawalParams {
        ...
        withdrawal_amount: DEFAULT_WITHDRAWAL_AMOUNT,
...
mod tests {
    ...
    #[test]
    fn only_the_exact_close_prefix_is_refused() {
        assert!(refuse_close_family_prefix("close_").is_err());
        for prefix in ["", "sepolia_", "c2c_", "close", "close_extra_", "burn_"] {
            assert!(
                refuse_close_family_prefix(prefix).is_ok(),
                "unexpected refusal for prefix {prefix:?}"
            );
        }
        assert_eq!(DEFAULT_DEPOSIT_AMOUNT - DEFAULT_WITHDRAWAL_AMOUNT, DEFAULT_WITHDRAWAL_AMOUNT);
    }
}
