...
impl U256 {
    ...
    pub fn rand_small<R: Rng>(rng: &mut R) -> Self {
        let mut limbs = rng.r#gen::<[u32; 6]>().to_vec();
...
impl U256 {
    pub fn rand<T: Rng>(rng: &mut T) -> Self {
        let limbs: [u32; U256_LEN] = rng.r#gen();
        Self { limbs }
    }
}
...
impl U256Target {
    pub fn add<F: RichField + Extendable<D>, const D: usize>(
        ...
        for (a, b) in self.limbs.iter().rev().zip(other.limbs.iter().rev()) {
            let (new_limb, new_carry) =
                builder.add_many_u32(&[carry, U32Target(*a), U32Target(*b)]);
            carry = new_carry;
    ...
    pub fn is_le<F: RichField + Extendable<D>, const D: usize>(
        ...
        list_le_circuit(
            builder,
            self.limbs.iter().rev().cloned().collect(),
            other.limbs.iter().rev().cloned().collect(),
            32,
        )
    }
