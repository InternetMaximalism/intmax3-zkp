...
// VENDOR EDIT (type adaptation only): upstream `rand` 0.10 is the host crate's `rand010` alias;
// upstream's `crate::utils::zeroize` re-export is provided by `falcon_sig::compat::zeroize`.
use rand010::Rng;

use super::{fft::FastFft, polynomial::Polynomial, samplerz::sampler_z};
use crate::falcon_sig::compat::zeroize::{Zeroize, ZeroizeOnDrop};
...
mod tests {
    ...
    // VENDOR EDIT (type adaptation only): rand 0.10 / rand_chacha 0.10 under host aliases.
