...
// VENDOR EDIT (type adaptation only): upstream's `crate::Felt` (Miden Goldilocks) maps to the
// host crate's plonky2 `GoldilocksField` via `falcon_sig::compat`; the module path
// `crate::dsa::falcon512_poseidon2` becomes `crate::falcon_sig::vendor`; zeroize comes from
// `falcon_sig::compat::zeroize`. `plonky2::field::types::Field` is imported for the
// `from_canonical_u16` constructor used below (upstream: `Felt::from_u16`).
...
impl From<Polynomial<FalconFelt>> for Polynomial<Felt> {
    fn from(item: Polynomial<FalconFelt>) -> Self {
        let res: Vec<Felt> =
            item.coefficients.iter().map(|a| Felt::from_canonical_u16(a.value() as u16)).collect();
        Polynomial::new(res)
    }
}
...
impl Polynomial<FalconFelt> {
    ...
    pub fn to_elements(&self) -> Vec<Felt> {
        self.coefficients.iter().map(|&a| Felt::from_canonical_u16(a.value() as u16)).collect()
    }
...
mod tests {
    ...
    // VENDOR EDIT (test-only): upstream's `crate::rand::test_utils::prng_array` deterministic
    // byte expander is provided by `falcon_sig::compat`. The assertion below (negacyclic
    // reduction identity) is unchanged and holds for any deterministic input bytes.
    use crate::falcon_sig::compat::prng_array;
