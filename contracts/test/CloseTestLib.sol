...
///      challenge / finalize / cancel logic and the verifier's REAL 103-limb
...
///      whose `publicInputs` we set to the expected 103 limbs, while still letting us flip the
...
library CloseTestLib {
    /// Build a close `MleProof` carrying the (103) raw close limbs `limbs` as its `publicInputs`.
