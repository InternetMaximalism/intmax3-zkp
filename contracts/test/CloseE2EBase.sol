...
///         address must be baked into the withdrawal proof before that proof is generated. Seven
///         proof-free V2 configuration artifacts let this harness constructor-pin every circuit
///         adapter first (the seventh, `close_asset_backing_mle_config.json`, is the whole-vector
///         `CloseAssetBacking` statement the materializer verifies for the signer-independent exit). The four parent contracts then use fixed CREATE2 salts and initcodes.
///
/// @dev Adapter cores are ordinary CREATE deployments. The address-printer and lifecycle test live
///      in the same test contract and deploy the seven adapters in the same order from a fresh test
...
abstract contract CloseE2EBase is Test {
    ...
    // ── Fixture files (the close set, co-generated by generate_close_fixture) ──

    /// The seven constructor-pinned circuit adapters of one close-lifecycle deployment. `backing`
    /// is the whole-vector `CloseAssetBacking` statement the signer-independent exit attests; it
    /// is a DIFFERENT circuit from `close` (pinning the close adapter there would make every
    /// honest attestation fail closed).
    ...
    function _backingConfigJson() internal view returns (string memory) {
        return _readData("close_asset_backing_mle_config.json");
    }

    /// The proved whole-vector `CloseAssetBacking` compact proof over the lifecycle's final balance
    /// proof (`close_asset_backing_mle.json`, co-generated with the close set).
    function _backingJson() internal view returns (string memory) {
        return _readData("close_asset_backing_mle.json");
    }
    ...
    function _v2DeploymentConfigsReady() internal view returns (bool) {
        return _isSchemaFile("close_lifecycle_validity_mle_config.json", V2_CONFIG_SCHEMA_HASH)
            ...
            && _isSchemaFile("close_asset_backing_mle_config.json", V2_CONFIG_SCHEMA_HASH);
    }
    ...
    function _deployPinnedAdapters() private returns (PinnedAdapters memory adapters) {
        ...
        (, PinnedMleVerifierV2 backing) = FixtureLib.deployPinnedMleV2(_backingConfigJson());
        ...
        adapters = PinnedAdapters({
            ...
            backing: IPinnedMleVerifierV2(address(backing))
        });
    }
    ...
    function _materializerInitcode(address rollupAddr, IPinnedMleVerifierV2 backing)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(
            type(CloseFundingMaterializer).creationCode, abi.encode(IntmaxRollup(payable(rollupAddr)), backing)
        );
    }
    ...
    /// @notice Deploy the seven proof-free adapters in canonical order and predict the manager whose
    ...
    function predictManagerAddressFrom(string memory lifecycleJson) public returns (address managerAddr) {
        ...
        address materializerAddr = _predict(SALT_MATERIALIZER, _materializerInitcode(rollupAddr, adapters.backing));
    ...
    /// @notice Constructor-pin all seven circuit adapters, then atomically create the two parents,
        ...
        )
    {
        ...
        materializer = CloseFundingMaterializer(
            _deploy(SALT_MATERIALIZER, _materializerInitcode(address(rollup), adapters.backing))
        );
        ...
        // runbook: `... ChannelSettlementManager -> registerSettlementManager`). Registration
        // discovers the Manager's `closeFundingMaterializer()` and binds the channel to the
        // set-once close satellite; without it every `requestClose` reverts `NotBoundManager` and
        // the Manager's post-finality funding path (`authorizePartialWithdrawal` /
        // `pullChannelFunds`) reverts `NotRegisteredSettlementManager`, so the lifecycle E2E would
        // stop before exercising the value path. The CREATE2 factory is the Rollup's deployer.
        vm.prank(FACTORY);
