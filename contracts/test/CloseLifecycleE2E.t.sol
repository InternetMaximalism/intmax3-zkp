...
contract CloseLifecycleE2ETest is CloseE2EBase {
    ...
    function setUp() public {
        ...
        // printer below needs only the seven config artifacts; the value-path test additionally
        // requires all FOUR witness-specific proof fixtures plus their descriptor/payout data: the
        // validity/withdrawal/close-intent proofs AND the whole-vector `CloseAssetBacking` proof
        // (`close_asset_backing_mle.json`) — all co-generated by
        ...
        ready = _v2DeploymentConfigsReady() && _isSchemaFile("close_lifecycle_validity_mle.json", V2_FULL_SCHEMA_HASH)
            ...
            && _isSchemaFile("close_asset_backing_mle.json", V2_FULL_SCHEMA_HASH)
        ...
        // Constructor-pin seven distinct V2 adapters and deploy/register the lifecycle stack
        // through the same deterministic path used by the address printer. The materializer is
        // pinned to the REAL CloseAssetBacking adapter (a DIFFERENT circuit from the close
        // circuit: the close adapter there would make every honest attestation fail closed).
    ...
    function _backingPublicInputs() internal view returns (uint256[] memory pi) {
        ...
        pi = new uint256[](raw.length);
        for (uint256 i = 0; i < raw.length; i++) {
            pi[i] = vm.parseUint(raw[i]);
        }
    }
    ...
    function _attestRealBacking() internal {
        ...
        // The pinned backing adapter must authenticate exactly the fixture's public inputs; a
        // close-adapter / stale-config substitution fails here, not deep inside the attestation.
        uint256[] memory authenticated = materializer.backingMleVerifier().verifyCompactPublicInputs(bproof);
        assertEq(keccak256(abi.encode(authenticated)), keccak256(abi.encode(pi)), "backing adapter PI != fixture PI");
    ...
    /// Print the close-manager CREATE2 address for the fixture-regeneration flow. The seven V2
    ...
    ///   WD_RECIPIENT=<addr> cargo run --release --features close-fixture-bin --bin generate_close_fixture
    /// which CO-GENERATES the whole close family (lifecycle set, close intent, backing proof).
    ...
    function test_closeLifecycle_endToEnd() public {
        ...
        // pinned V2 verifiers. The withdrawal-claim BINDING + payout is exercised independently by
        // `ChannelSettlementManager.t.sol` (real 48-limb strict bind) — but note that suite wires
        // `MockPinnedMleVerifierV2`, which accepts unconditionally, so it covers binding ONLY and says
